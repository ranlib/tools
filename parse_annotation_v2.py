...
parser.add_argument("--json", "-j", help="json file with mutation loci", required=True)
...
with open(args.loci, "r") as csvfile:
    reader = csv.reader(csvfile, delimiter='\t')
    header = next(reader)
    for row in reader:
        genez.append(row[0])
        genezid.append(row[1])
        start.append(int(row[2]))
        stop.append(int(row[3]))
        gene_anot.append(row[4])
        strand.append(row[5])
...
header = ["Sample ID", "CHROM", "POS", "REF", "ALT", "Read Depth", "Percent Alt Allele", "Annotation", "Variant Type",
          ...
          "Codon Position", "Gene Name", "Gene ID", "Drug annotation"]
with open(args.output, "w") as output:
    ...
    with F as vcf:
        for lines in vcf:
            ...
            # add drug annotation to csv file
            record.append("|".join(drug_annotation))
